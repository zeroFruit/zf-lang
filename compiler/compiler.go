...
func (c *Compiler) Compile(node ast.Node) error {
	...
	case *ast.InfixExpression:
		// since this the operator for which we want to reorder the operands
		// its implementation is an addition right at the beginning
		if node.Operator == "<" {
			if err := c.Compile(node.Right); err != nil {
				return err
			}
...
// changeOperand underlying assumption is that we only replace instructions of the same type
